...
package net.mcreator.mindustryinminecraft.keybind;
...
public class ConveyermakerkeyKeyBinding extends MindustryinminecraftModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
	private KeyBinding keys;
	public ConveyermakerkeyKeyBinding(MindustryinminecraftModElements instance) {
		super(instance, 28);
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					MindustryinminecraftMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
		...
		if (type == 0) {
			{
				...
				ConveyermakerkeyOnKeyPressedProcedure.executeProcedure($_dependencies);
			}
		}
	}
}
